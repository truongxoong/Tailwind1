...
let cartTop = document.querySelector('#valueTop')
...
function addProductCart(proImg, proName, proPrice) {
  let cartProductList = document.querySelector('.cartProduct')
    ...
    cartProduct.innerHTML = '<div class="elementPro grid grid-cols-2 gap-2 py-5 border-b">    <div class=" w-[110px] h-[130px]  m-auto"> <img class=" w-full h-full object-cover " src="'+proImg+'" alt=""></div><div class="flex justify-between flex-col"><div><h4 class="">'+proName+'</h4><p class="text-sm text-gray-500">$<span class="pricePro">'+proPrice+'</span></p></div><div><input class="w-[50px] border-0 focus:outline-none pl-1" type="number" value="1" min="1"></div><div><i class="fa-solid fa-pen-to-square"></i><i class="fa-solid fa-trash-can pl-[10px] delete"></i></div></div></div>'
...
function totalPrice() {
  let totalPri = 0
  let valuePro = 0
  let cartProductList = document.querySelectorAll('.elementPro')
  ...
  let valueCartTop = document.querySelector('.valueTop')
  valueCartTop.innerHTML = valuePro
}
// hàm delete product
function deleteProduct() {
  let cartProductList = document.querySelectorAll('.elementPro')
  cartProductList.forEach((e) => {
    let deletePro = e.querySelector('.delete')
    deletePro.addEventListener('click', function(ele) {
...
function changeProduct() {
  let cartProductList = document.querySelectorAll('.elementPro')
  cartProductList.forEach((e) => {
    let change = e.querySelector('input')
